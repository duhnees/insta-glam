        ...
        <div className="flex border rounded w-fit p-4">
            { postInfo && <div className="space-y-8">
                <div className="space-y-4 float-left w-80">
                    ...
                    {!commenting && <button className="btn bg-teal-800 text-white font-bold rounded w-full h-[45px]" onClick={() => {
                        if(!loggedIn) {navigate('/login');} else {setCommenting(true)}
                    }}>Comment</button>}
                    {commenting && <CommentPopup onChange={setCommenting} postId={postId}/>}
